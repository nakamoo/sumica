...
public:
    WUserInput(std::string& directoryPath) :
        ...
        mCounter{0}, mDirectory{directoryPath}
    {
    ...
    std::shared_ptr<std::vector<op::Datum>> workProducer()
    {
        try
        {
            //op::error(mImageFiles, __LINE__, __FUNCTION__, __FILE__); 
            //cout << mImageFiles << endl;
            ...
            if (mImageFiles.size() <= mCounter)
            {
                ...
                //this->stop();
                std::string mDir = mDirectory;
                mImageFiles = op::getFilesOnDirectory(mDirectory, "png");
                mCounter = 0;
                return nullptr;
            }
            else
            {
                ...
                auto path = mImageFiles.at(mCounter);
                mCounter++;
                op::log(path, op::Priority::High);

                // Fill datumi
                datum.cvInputData = cv::imread(path);
                datum.name = path.substr(path.find_last_of("\\/"));
                datum.name = datum.name.substr(0, datum.name.find_last_of("."));
